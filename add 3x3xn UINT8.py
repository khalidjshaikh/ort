# python onnx onnxruntime add 3x3x1000000 matrix ir_version 10 opset_version 22 UINT8
...
# input_data_X = np.random.rand(3, 3, n).astype(np.uint8)
# input_data_Y = np.random.rand(3, 3, n).astype(np.uint8)
input_data_X = ones((n,3,3)).astype(uint8)
input_data_Y = ones((n,3,3)).astype(uint8) *2
# input_data_X = zeros((3,3, int(1e6))).astype(uint8)
